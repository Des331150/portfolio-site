...
function renderProjects(container, repos) {
    ...
    repos.forEach(repo => {
        ...
        accentBar.style.cssText = 'height: 3px; background-color: ' + langColor
        card.appendChild(accentBar)

        const body = document.createElement('div')
        body.className = 'tw-flex tw-flex-col tw-gap-2 tw-p-4'

        const header = document.createElement('div')
        header.className = 'tw-flex tw-items-center tw-gap-2'

        const icon = document.createElement('i')
        icon.className = 'bi bi-github tw-text-xl'
        ...
        desc.className = 'tw-text-sm tw-text-gray-500 dark:tw-text-gray-400 tw-line-clamp-2'
        desc.textContent = repo.description || 'No description'
        body.appendChild(desc)

        const footer = document.createElement('div')
        footer.className = 'tw-mt-auto tw-flex tw-items-center tw-gap-4 tw-text-sm tw-text-gray-600 dark:tw-text-gray-400'

        const langSpan = document.createElement('span')
        langSpan.className = 'tw-flex tw-items-center tw-gap-1'

        const dot = document.createElement('span')
        dot.style.cssText = 'display: inline-block; width: 10px; height: 10px; border-radius: 50%; background-color: ' + langColor
        langSpan.appendChild(dot)

        langSpan.appendChild(document.createTextNode(' ' + (lang || 'N/A')))
